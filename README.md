...
- **高级算法矩阵**：新增 A*/RCSP 标签搜索、SciPy MILP 校验、鲁棒场景搜索、MCTS/遗传/退火启发式和多人博弈对照实验。
...
flowchart LR
    ...
    D --> J["benchmark: 高级算法矩阵"]
    E --> G["solutions / Result_solved.xlsx"]
    F --> H["report_tables"]
    J --> K["algorithm_benchmark / comparison"]
...
高级算法矩阵保留当前 DP 求解器作为提交基线，同时增加：

- `astar_dp`：带到终点最短距离下界的启发式标签搜索实验。
- `rcsp_label`：资源约束最短路视角的非支配标签搜索实验。
- `milp_exact`：基于时间扩展网络的 SciPy MILP 精确/近精确校验。
- `robust_rcsp`：未知天气场景集合上的最坏收益、均值收益和失败率比较。
- `mcts_rollout`、`ga_search`、`sa_search`：固定随机种子的智能优化对照组。
- `coalition_search`、`best_response_check`：第五、六关多人合作和偏离收益检查。
...
/opt/miniconda3/envs/pytorch_env/bin/python run_desert_model.py benchmark --levels all
...
python run_desert_model.py benchmark --levels all
...
```bash
/opt/miniconda3/envs/pytorch_env/bin/python run_desert_model.py benchmark --levels 1,2,3,4 --budget 6 --scenario-limit 6
```
...
| `output/experiments/algorithm_benchmark.csv` | 高级算法对比实验表 |
| `output/experiments/algorithm_benchmark.json` | 高级算法结果、元数据和轨迹路径 |
| `output/report_tables/algorithm_comparison.md` | 可直接放入论文的算法对比摘要 |
...
- 确定性求解器与多人策略摘要。
- 高级算法 runner 输出结构、MILP/A*/RCSP 小图一致性和固定随机种子启发式。
